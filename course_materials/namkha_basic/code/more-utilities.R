...
plot_detfunction <- function(model, seed = 1, nsim = 1000, nd = 200, nbins = 25,
                             modelname = NA, lam0_covs_vals = numeric(0)){
  
  if (!(model$detectfn %in% c(0, 1, 2, 14, 15, 16)))
    stop("Detection function not supported")
  
  parnames <- row.names(coefficients(model))
  lam0_terms <- parnames[grepl("^lambda0", parnames)]
  lam0_cov_terms <- lam0_terms[lam0_terms != "lambda0"]
  
  if (length(lam0_covs_vals) == 0) {
    lam0_covs_vals <- rep(0, length(lam0_cov_terms))
  }
  
  if (length(lam0_covs_vals) != length(lam0_cov_terms)) {
    stop("lam0_covs_vals must have one value for each lambda0 covariate in the model.")
  }
  
  lambda0_linear_predictor <- function(pars) {
    eta <- pars["lambda0"]
    if (length(lam0_cov_terms) > 0) {
      eta <- eta + sum(pars[lam0_cov_terms] * lam0_covs_vals)
    }
    eta
  }
  
  sigmahat = exp(coef(model)["sigma", "beta"])
  if(model$detectfn %in% c(14,15,16)){
    lambda0 = exp(lambda0_linear_predictor(coef(model)[, "beta"]))
    g0 = 1 - exp(-lambda0)
  } else if(model$detectfn %in% c(0,1,2)){
    g0 = logit(coef(model)["g0", "beta"])
    lambda0 = -log(1 - g0)
  } 
  ...
  lambdaHR = function(pars,d) 
    exp(lambda0_linear_predictor(pars))*(1-exp(-(d/exp(pars["sigma"]))^(-exp(pars["z"]))))
  lambdaHN = function(pars,d) 
    exp(lambda0_linear_predictor(pars))*exp(-0.5*(d/exp(pars["sigma"]))^2)
  lambdaHEX <- function(pars, d)
    exp(lambda0_linear_predictor(pars)) * exp(-(d / exp(pars["sigma"])))
  ...
  df2lambdaHEX <- function(pars, d) {
    p <- plogis(pars["g0"]) * exp(-(d / exp(pars["sigma"])))
    -log(1 - p)
  }
  
  # Calculate the point estimates of the encounter rate and associated detection function
  dmax = max(unlist( moves(model$capthist) )) * 1.01
  ...
  } else if (model$detectfn == 16) {          # <‑‑ new (λ‑scale)
    lambda.est <- lambdaHEX(mu, d)
    p.est      <- 1 - exp(-lambda.est)
  ...
  } else if (model$detectfn == 2)  {          # <‑‑ new (g0‑scale)
    lambda.est <- df2lambdaHEX(mu, d)
    p.est      <- 1 - exp(-lambda.est)
  }
  ...
  } else if (model$detectfn == 16){
    for(i in 1:nsim) {
      lambda.ests[i,] = lambdaHEX(pars[i,],d)
      p.ests[i,] = 1 - exp(-lambda.ests[i,])
    }
  ...
  } else if (model$detectfn == 2)  {          
    for(i in 1:nsim) {
      lambda.ests[i,] = df2lambdaHEX(pars[i,],d)
      p.ests[i,] = 1 - exp(-lambda.ests[i,])
    }
  }
  ...
  ## observed distances of spatial recaptures
  obsdist <- unlist( moves(model$capthist) )
  
  ## pick the same distance range you used for the curve
  breaks   <- seq(0, max(d), length.out = nbins + 1)
  
  ## histogram without plotting – density so that area = 1
  h        <- hist(obsdist, breaks = breaks, plot = FALSE)
  
  ## numeric area under the encounter‑rate curve (trapezoidal)
  area_enc <- sum( diff(d) * (lambda.est[-1] + lambda.est[-length(lambda.est)]) / 2 )
  
  ## scale histogram density so its total area matches area_enc
  h$dens.scaled <- h$density * area_enc
  ...
  return( list(detfun = df_detfun,
               hist   = data.frame( xmid = h$mids,
                                    height = h$dens.scaled,
