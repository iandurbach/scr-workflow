...
# of the survey region and predicted activity-centre density for the short survey.
# Script to run before this one: 04b_model_fitting_short.R
...
results_dir <- here("namkha_advanced", "output", "results", "short")
...
models_file <- here("namkha_advanced", "output", "namkha_advanced_fitted_models_short.RData")
...
traps_sf <- st_as_sf(short_traps, coords = c("x", "y"), crs = my_crs)
...
ggsave(file.path(results_dir, "namkha_advanced_survey_region_short.png"), survey_region_plot, width = 7, height = 5, dpi = 200)
ggsave(file.path(results_dir, "namkha_advanced_predicted_density_short.png"), density_plot, width = 7, height = 5, dpi = 200)

# Save the predicted density surface as a shapefile for later mapping
st_write(
  density_cells %>% dplyr::select(D_per_100km2),
  dsn = file.path(results_dir, "namkha_advanced_predicted_density_short.shp"),
  delete_layer = TRUE,
  quiet = TRUE
)
...
# s4 includes the density and detection covariates used in these example plots.
covplot_model <- fitted_models[["s4"]]
...
ggsave(file.path(results_dir, "namkha_advanced_Dcovs_short.png"), pD, width = 9, height = 3.5, dpi = 300)
...
# For s4, c(1, 0) = cliff, c(0, 1) = valley_stream, c(0, 0) = other/baseline
# First baseline plot
...
ggsave(file.path(results_dir, "namkha_advanced_Ecovs_short_baseline.png"), ec1, width = 9, height = 4.5, dpi = 300)
...
# Save the detection covariate plot for later reference
ggsave(file.path(results_dir, "namkha_advanced_Ecovs_short_cliff.png"), ec1, width = 9, height = 4.5, dpi = 300)
...
# Save the detection covariate plot for later reference
ggsave(file.path(results_dir, "namkha_advanced_Ecovs_short_valley_stream.png"), ec1, width = 9, height = 4.5, dpi = 300)
